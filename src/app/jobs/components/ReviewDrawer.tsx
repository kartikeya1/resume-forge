 ...
 * confidently. Roving-tabindex arrow-key navigation, matching the manual
 * keydown-listener pattern already used by Menu in src/components/ui.tsx -
 * there is no ready-made list-nav primitive in this repo to reuse instead.
 *
  ...
  return (
    <section aria-labelledby="review" className="mt-6">
      <h2 id="review" className="mb-1 text-sm font-semibold text-neutral-900 dark:text-neutral-100">
        Needs a human look ({visible.length})
      </h2>
      <p className={`mb-2 text-xs ${MUTED}`}>
        Mail that looks job-related but could not be placed confidently. Arrow keys to move,{' '}
        <kbd className="rounded border border-neutral-300 px-1 dark:border-neutral-600">a</kbd> to add as
        an application, <kbd className="rounded border border-neutral-300 px-1 dark:border-neutral-600">x</kbd>{' '}
        to dismiss, <kbd className="rounded border border-neutral-300 px-1 dark:border-neutral-600">Enter</kbd>{' '}
        to open in Gmail.
      </p>
      <div className={`${CARD} overflow-hidden`} role="listbox" aria-label="Threads needing review">
        <ul>
          {visible.map((t, i) => (
            <li
              key={t.threadId}
              ...
              onKeyDown={(e) => {
                if (e.key === 'ArrowDown') { e.preventDefault(); focusIndex(i + 1); }
                ...
                else if (e.key === 'Enter' || e.key === 'o') { window.open(gmailThreadUrl(t.threadId), '_blank', 'noreferrer'); }
                else if (e.key === 'a') {
                  ...
                  // Focus doesn't move on its own once the row unmounts, so
                  // step to the next one deliberately.
                  requestAnimationFrame(() => focusIndex(i));
                }
              }}
              className={`flex items-start justify-between gap-3 border-t border-neutral-100 px-3 py-2 text-xs first:border-t-0 outline-none dark:border-neutral-700 ${
